...
//   Api to toggle Car Availbility
export const  toggleCarAvailbility = async (req, res) =>{
  try {
    const { _id } = req.user;
...
export const  getDashboardData = async (req, res) =>{
       try {
         ...
         const cars = await Car.find({owner: _id})
          const booking = await Booking.find({owner: _id}).populate('çar').
          sort({createdAt: -1});
        ...
        const monthlyRevenue = booking.slice().filter(booking => booking.
          status === 'confirmed').reduce((acc, booking)=>acc + booking.price, 0)

          const dashboardData = {
              totalCars: car.length,
              totalBookings: booking.lenghth,
